...
export default function EditEmployee({ empID }: {empID: Number | null | undefined}) {
...
const submitHandleClose = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/v1/employees/${employeeId}/edit`,
      ...
      setloading(false);
      setOpen(false);
      toast(`${response.data.message} empId is ${employeeId}`);
    } catch (error) {
  ...
  return (
    <>
      ...
      <React.Fragment>
        ...
        >
          <DialogTitle className="font-bold">{"Edit Employee"}</DialogTitle>
          <DialogContent>
